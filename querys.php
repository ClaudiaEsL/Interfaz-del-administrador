...
<div class="content">
        ...
        <div class="contenido"><div class="sesion d-flex"><?php require 'components/sesion.php';?></div>
            ...
            <div class="container" style="display: flex; justify-content:center;">
                <form class="row bg-light border" method="POST" action="querys.php" style="margin-top:1%;margin-bottom:1%;">
                    <div class="col-md-6">
                        <select class="form-select form-select" name="buscar" id="inputpersonas" required >
                        <option selected disabled>-Elija a un jugador-</option>
                        <?php
                            /*Cargar datos de jugadores*/
                            $consulta =  mysqli_query($conn,"SELECT id_jugador, CONCAT(nombre,' ',ap_paterno,' ', ap_materno) nombre_completo FROM jugador ORDER BY nombre");
                            while($personas = mysqli_fetch_array($consulta)){
                        ?>
                            <option value="<?= $personas['id_jugador']?>"><?= $personas['nombre_completo']?></option>
                        <?php
                            }
                        ?>
                        </select>
            ...
            <?php if(empty($verificacion)):?>
                ...
                <svg style="color:gray;" xmlns="http://www.w3.org/2000/svg" width="60" height="60" fill="currentColor" class="bi bi-search" viewBox="0 0 16 16"><path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001c.03.04.062.078.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1.007 1.007 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0z"/></svg>
                ...
                <div class="text-center" style="height:500px;width: 100%;display: flex; justify-content:center;">
                    <div class="border" style="display:flex;padding:1;max-width: 600px;max-height:400px;text-align:center;">
                        ...
                        <div class="card-body bg-light">
                            ...
                            <p class="card-text"><strong>Posicion:</strong>  <?= $encontrado['posicion'];?> </p>
                            <p class="card-text"><strong>Categoria:</strong>  <?= $encontrado['categoria'];?> </p>
                            <p class="card-text"><strong>Lugar de nacimiento:</strong>  <?= $encontrado['lugar_nac'];?> </p>
                            <p class="card-text"><strong>Fecha de nacimiento:</strong>  <?= $encontrado['fecha_nac'];?> </p>
                            <?php echo "<a title=Ver test' class='btn btn-primary' style='color:white; margin-left:1%;' href='test_list_save.php?id_jugador=".$encontrado['id_jugador']."'>Ver test registrados</a>"; ?>
                        </div>
                    </div>
                </div>
            <?php endif;?>
